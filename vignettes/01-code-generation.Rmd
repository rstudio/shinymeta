...
server <- function(input, output, session) {
  ...
  output$code <- renderPrint({
    expandChain(output$plot(), .pkgs = "ggplot2")
  })
}
...
## Expanding meta objects

Now that we've translated our Shiny reactive expressions to their **shinymeta** variants, [unquoted](#unquoting) reads of reactive values and expressions, and leveraged `metaExpr()` to further avoid Shiny specific logic, we can start extracting the code with `expandChain()`. By default, it will give you all the code behind all the outputs in your app:
...
TODO: an example of calling `expandChain()` twice, but in a way that doesn't duplicate logic.


Sometimes it won't make sense to provide the entire path of execution

In some cases, you may want to short-cut the reactive path:

For example, say your app has a 
