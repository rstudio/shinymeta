...
However, when invoked with **shinymeta**'s meta-mode enabled (i.e., `withMetaMode()`), the behavior changes: Instead of evaluating the reactive expression and returning the value (a data frame), it returns a code expression. To make it visually clear when we're generating a code expression, this article sets the background color of generated code to <svg width="1em" height="1em"><rect width="1em" height="1em" style="fill: #d3f3df"/></svg>.
...
To ignore the `validate` and `need()` calls, change `metaRender()` to `metaRender2()`, then wrap the plot generation code in `metaExpr()`. It's worth noting that, with any of the `-2` variants, _only the code inside of a `metaExpr()` is evaluated with meta-mode enabled_, which means, in the example below, `downloads()` always returns a data frame, and never the code expression that generates it.
