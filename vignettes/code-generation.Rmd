...
* Each read of a meta-reactive or an `input` value has been surrounded with `.()`.
...
However, `metaReactive()`s have the added ability to return code expressions when invoked in "meta mode". **shinymeta** currently provides two ways to invoke reactives in meta mode: `withMetaMode()` and `expandChain()`. In practice, you'll almost always want to use `expandChain()` over `withMetaMode()`: it has the added ability of assigning return values to a name and reusing that name when necessary, which helpful for avoiding redundant code and computation when [expanding a chain of reactives](#chaining).

```{r}
expandChain(downloads())
```

Notice how the generated code here depends on the input variable `input$package`. If we were to run this code in a new R session, it would error because `input$package` isn't defined outside of the shiny session. We can, however, replace the literal name "`input$package`" with the value it represents (by 'unquoting' it with `.()`), which allows the code to run an environment where `input` isn't defined. 
...
> #### Takeaway: replace reactive inputs/values with their static value via `.()`
...
When meta-reactives are invoked in meta-mode, they perform quasi-quotation by evaluating terms wrapped in `.()` (like `bquote()` does).^[When invoked normally, the `.()` is ignored, ensuring normal execution behaves as you'd expect it to.] We've already seen how `.()` can be used to replace reactive inputs with their static values (**note**: this also works with `reactiveVal()`/`reactiveValues()`), but this can also be useful for customizing non-reactive aspects of the code that you generate.

For example, by unquoting the value supplied to the `from` argument in `downloads()`, we'll inline the value that the code expression `Sys.Date() - 365` represents, so that when the generated code is run in the future, we always get downloads starting `from` a year prior to *the day the code was generated* (instead of a year prior to *the day the generated code is run*).
...
> #### Takeaway: (optionally) unquote non-reactive code to further control logic in generated code 

In summary, unquoting replaces a name with the value that it represents; and thus, is a essential tool for generating code that can run outside of a shiny session. That's because, in a Shiny app, we often refer to reactive values that are only defined inside a shiny runtime. Unquoting is fairly straightforward when the name you're unquoting (`e.g. input$package`) represents a constant value (e.g. `"ggplot2"`), but is a bit more complicated when those values become more complicated. In the next section, we'll be chaining meta-reactive expressions, which requires unquoting names that represent meta-reactives. That means, these names can actually be thought of as un-evaluated expressions!
...
Recall that, when invoked with meta-mode enabled, `metaReactive()`, like `bquote()`, quotes its input, which is why `downloads()` appears in the code produced by `downloads_rolling()`:
...
As a side-note, when unquoting a read of a reactive expression, you should make sure the `!!` operator applies to the read, and nothing else. A common scenario where this subtlety becomes relevant is when using `$` or `[[` operators to subset the return value:

```{r, error = TRUE}
downloads_count <- metaReactive({
  !!downloads()$count
})
expandChain(downloads_count())
```

In this case, you can surround the unquoting operator in parentheses to ensure the unquoting applies only to `downloads()` (instead of `downloads()$count`): 
...
There are a few other scenarios where landing upon the proper quasiquotation strategy isn't very straightforward. For example, what if you want to generate code that contains the unquote operator (`!!`) for other [tidyeval](https://tidyeval.tidyverse.org/) purposes (e.g. [programming with dplyr](https://dplyr.tidyverse.org/articles/programming.html))? to learn more about these scenarios, and how to manage them, see the [special topics](special-topics.html) article.
